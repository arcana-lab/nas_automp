...
#pragma omp for
#pragma note noelle selected = 1 label = 1
...
c-------------------------------------------------------------------*/
#pragma omp for
#pragma note noelle selected = 1 label = 3
  for (j = 1; j < grid_points[1]-1; j++) {
    eta = (double)j * dnym1;
...
c-------------------------------------------------------------------*/
#pragma omp for
#pragma note noelle selected = 1 label = 4
  for (i = 1; i < grid_points[0]-1; i++) {
    xi = (double)i * dnxm1;
...
c-------------------------------------------------------------------*/
#pragma omp for
#pragma note noelle selected = 1 label = 6
  for (i = 1; i < grid_points[0]-1; i++) {
    xi = (double)i * dnxm1;
...
c-------------------------------------------------------------------*/
#pragma omp for  
#pragma note noelle selected = 1 label = 7
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
      for (k = 1; k < grid_points[2]-1; k++) {
	for (m = 0; m < 5; m++) {
...
c-------------------------------------------------------------------*/
#pragma omp for
#pragma note noelle selected = 1 label = 8
  for (i = 0; i < IMAX; i++) {
    for (j = 0; j < IMAX; j++) {
      for (k = 0; k < IMAX; k++) {
	for (m = 0; m < 5; m++) {
...
#pragma omp for
#pragma note noelle selected = 1 label = 9
  for (i = 0; i < grid_points[0]; i++) {
    xi = (double)i * dnxm1;
...
#pragma note noelle selected = 1 label = 10
  for (j = 0; j < grid_points[1]; j++) {
    eta = (double)j * dnym1;
    for (k = 0; k < grid_points[2]; k++) {
      zeta = (double)k * dnzm1;
...
c-------------------------------------------------------------------*/
  ...
  xi = 1.0;
#pragma omp for
#pragma note noelle selected = 1 label = 11
  for (j = 0; j < grid_points[1]; j++) {
    eta = (double)j * dnym1;
    for (k = 0; k < grid_points[2]; k++) {
      zeta = (double)k * dnzm1;
...
c-------------------------------------------------------------------*/
  ...
  eta = 0.0;
#pragma omp for nowait
#pragma note noelle selected = 1 label = 12
  for (i = 0; i < grid_points[0]; i++) {
    xi = (double)i * dnxm1;
    for (k = 0; k < grid_points[2]; k++) {
      zeta = (double)k * dnzm1;
...
c-------------------------------------------------------------------*/
  ...
  eta = 1.0;
#pragma omp for
#pragma note noelle selected = 1 label = 13
  for (i = 0; i < grid_points[0]; i++) {
    xi = (double)i * dnxm1;
    for (k = 0; k < grid_points[2]; k++) {
      zeta = (double)k * dnzm1;
...
c-------------------------------------------------------------------*/
  ...
  zeta = 0.0;
#pragma omp for nowait
#pragma note noelle selected = 1 label = 14
  for (i = 0; i < grid_points[0]; i++) {
    xi = (double)i *dnxm1;
    for (j = 0; j < grid_points[1]; j++) {
      eta = (double)j * dnym1;
...
c-------------------------------------------------------------------*/
  ...
  zeta = 1.0;
#pragma omp for
#pragma note noelle selected = 1 label = 15
  for (i = 0; i < grid_points[0]; i++) {
    xi = (double)i * dnxm1;
    for (j = 0; j < grid_points[1]; j++) {
      eta = (double)j * dnym1;
...
c-------------------------------------------------------------------*/
#pragma omp for
#pragma note noelle selected = 1  label = 16
  for (i = 0; i < grid_points[0]; i++) {
    for (j = 0; j < grid_points[1]; j++) {
      for (k = 0; k < grid_points[2]; k++) {
	for (m = 0; m < 5; m++) {
...
c-------------------------------------------------------------------*/
#pragma omp for  
#pragma note noelle selected = 1  label = 17
  for (i = 0; i < grid_points[0]; i++) {
    for (j = 0; j < grid_points[1]; j++) {
      for (k = 0; k < grid_points[2]; k++) {
	for (m = 0; m < 5; m++) {
...
c-------------------------------------------------------------------*/
#pragma omp for  
#pragma note noelle selected = 1 label = 18
  for (j = 1; j < grid_points[1]-1; j++) {
    for (k = 1; k < grid_points[2]-1; k++) {
      for (i = 0; i < grid_points[0]; i++) {
...
c-------------------------------------------------------------------*/
#pragma omp for
#pragma note noelle selected = 1 label = 19
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 0; j < grid_points[1]; j++) {
      for (k = 1; k < grid_points[2]-1; k++) {
...
c-------------------------------------------------------------------*/
#pragma omp for  
#pragma note noelle selected = 1 label = 20
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
      for (k = 1; k < grid_points[2]-1; k++) {
...
c---------------------------------------------------------------------*/
#pragma omp for
#pragma note noelle selected = 1 label = 21
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
      for (k = 0; k < grid_points[2]; k++) {
...
c-------------------------------------------------------------------*/
#pragma omp for
#pragma note noelle selected = 1 label = 22
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
      for (k = 1; k < grid_points[2]-1; k++) {
...
c-------------------------------------------------------------------*/
#pragma omp for nowait
#pragma note noelle selected = 1 label = 23// YES
  for (i = 0; i < grid_points[0]; i++) {
    for (j = 0; j < grid_points[1]; j++) {
      for (k = 0; k < grid_points[2]; k++) {
...
#pragma omp for
#pragma note noelle selected = 1 label = 24
  for (i = 0; i < grid_points[0]; i++) {
    for (j = 0; j < grid_points[1]; j++) {
      for (k = 0; k < grid_points[2]; k++) {
	for (m = 0; m < 5; m++) {
...
c-------------------------------------------------------------------*/
#pragma omp for
#pragma note noelle selected = 1 label = 25// YES?
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
      for (k = 1; k < grid_points[2]-1; k++) {
	uijk = us[i][j][k];
...
c-------------------------------------------------------------------*/
  i = 1;
#pragma omp for nowait
#pragma note noelle selected = 1 label = 26// YES?
  for (j = 1; j < grid_points[1]-1; j++) {
    for (k = 1; k < grid_points[2]-1; k++) {
      for (m = 0; m < 5; m++) {
  ...
  i = 2;
#pragma omp for nowait
#pragma note noelle selected = 1 label = 27// YES ?
  for (j = 1; j < grid_points[1]-1; j++) {
    for (k = 1; k < grid_points[2]-1; k++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m] - dssp * 
...
#pragma omp for nowait
#pragma note noelle selected = 1 label = 28
  ...
  i = grid_points[0]-3;
#pragma omp for nowait
#pragma note noelle selected = 1 label = 29// YES?
  for (j = 1; j < grid_points[1]-1; j++) {
    for (k = 1; k < grid_points[2]-1; k++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m] - dssp *
  ...
  i = grid_points[0]-2;
#pragma omp for
#pragma note noelle selected = 1 label = 30
  for (j = 1; j < grid_points[1]-1; j++) {
    for (k = 1; k < grid_points[2]-1; k++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m] - dssp *
...
c-------------------------------------------------------------------*/
#pragma omp for
#pragma note noelle selected = 1 label = 31
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
      for (k = 1; k < grid_points[2]-1; k++) {
	vijk = vs[i][j][k];
...
c-------------------------------------------------------------------*/
  j = 1;
#pragma omp for nowait
#pragma note noelle selected = 1 label = 32
  for (i = 1; i < grid_points[0]-1; i++) {
    for (k = 1; k < grid_points[2]-1; k++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m]- dssp * 
  ...
  j = 2;
#pragma omp for nowait
#pragma note noelle selected = 1 label = 33
  for (i = 1; i < grid_points[0]-1; i++) {
    for (k = 1; k < grid_points[2]-1; k++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m] - dssp * 
...
#pragma omp for nowait
#pragma note noelle selected = 1 label = 34
  for (i = 1; i < grid_points[0]-1; i++) {
  ...
  j = grid_points[1]-3;
#pragma omp for nowait
#pragma note noelle selected = 1 label = 35
  for (i = 1; i < grid_points[0]-1; i++) {
    for (k = 1; k < grid_points[2]-1; k++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m] - dssp *
  ...
  j = grid_points[1]-2;
#pragma omp for
#pragma note noelle selected = 1 label = 36
  for (i = 1; i < grid_points[0]-1; i++) {
    for (k = 1; k < grid_points[2]-1; k++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m] - dssp *
...
c-------------------------------------------------------------------*/
#pragma omp for
#pragma note noelle selected = 1 label = 37
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
      for (k = 1; k < grid_points[2]-1; k++) {
	wijk = ws[i][j][k];
...
c-------------------------------------------------------------------*/
  k = 1;
#pragma omp for nowait
#pragma note noelle selected = 1 label = 38
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m]- dssp * 
  ...
  k = 2;
#pragma omp for nowait
#pragma note noelle selected = 1 label = 39
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m] - dssp * 
...
#pragma omp for nowait
#pragma note noelle selected = 1 label = 40
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
  ...
  k = grid_points[2]-3;
#pragma omp for nowait
#pragma note noelle selected = 1 label = 41
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m] - dssp *
  ...
  k = grid_points[2]-2;
#pragma omp for
#pragma note noelle selected = 1 label = 42
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
      for (m = 0; m < 5; m++) {
	rhs[i][j][k][m] = rhs[i][j][k][m] - dssp *
...
#pragma omp for
#pragma note noelle selected = 1 label = 43
  for (j = 1; j < grid_points[1]-1; j++) {
    for (k = 1; k < grid_points[2]-1; k++) {
      for (m = 0; m < 5; m++) {
	for (i = 1; i < grid_points[0]-1; i++) {
...
#pragma omp for
#pragma note noelle selected = 1 label = 44
    for (j = 1; j < grid_points[1]-1; j++) {
      for (k = 1; k < grid_points[2]-1; k++) {
...
c-------------------------------------------------------------------*/
#pragma omp for
#pragma note noelle selected = 1 label = 45
  for (j = 1; j < grid_points[1]-1; j++) {
    for (k = 1; k < grid_points[2]-1; k++) {
...
c-------------------------------------------------------------------*/
  for (i = 1; i < isize; i++) {
#pragma omp for
#pragma note noelle selected = 1 label = 46
    for (j = 1; j < grid_points[1]-1; j++) {
      for (k = 1; k < grid_points[2]-1; k++) {
...
#pragma omp for
#pragma note noelle selected = 1 label = 47
  for (j = 1; j < grid_points[1]-1; j++) {
    for (k = 1; k < grid_points[2]-1; k++) {
...
#pragma omp for
#pragma note noelle selected = 1 label = 48
    for (i = 1; i < grid_points[0]-1; i++) {
      for (k = 1; k < grid_points[2]-1; k++) {
	for (m = 0; m < BLOCK_SIZE; m++) {
	  for (n = 0; n < BLOCK_SIZE; n++) {
...
#pragma omp for  
#pragma note noelle selected = 1 label = 49
  for (i = 1; i < grid_points[0]-1; i++) {
    for (k = 1; k < grid_points[2]-1; k++) {
...
c-------------------------------------------------------------------*/
  for (j = 1; j < jsize; j++) {
#pragma omp for
#pragma note noelle selected = 1 label = 50
    for (i = 1; i < grid_points[0]-1; i++) {
      for (k = 1; k < grid_points[2]-1; k++) {
...
#pragma omp for  
#pragma note noelle selected = 1 label = 51
  for (i = 1; i < grid_points[0]-1; i++) {
    for (k = 1; k < grid_points[2]-1; k++) {
...
#pragma omp for  
#pragma note noelle selected = 1 label = 52
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
...
c-------------------------------------------------------------------*/
#pragma omp for  
#pragma note noelle selected = 1 label = 53
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
...
c-------------------------------------------------------------------*/
  for (k = 1; k < ksize; k++) {
#pragma omp for
#pragma note noelle selected = 1 label = 54
    for (i = 1; i < grid_points[0]-1; i++) {
	  for (j = 1; j < grid_points[1]-1; j++) {
...
c-------------------------------------------------------------------*/
#pragma omp for  
#pragma note noelle selected = 1 label = 55
  for (i = 1; i < grid_points[0]-1; i++) {
    for (j = 1; j < grid_points[1]-1; j++) {
