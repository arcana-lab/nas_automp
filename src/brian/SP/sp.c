...
static void lhsx(void) {
  ...
  for (j = 1; j <= grid_points[1]-2; j++) {
    for (k = 1; k <= grid_points[2]-2; k++) {
#pragma omp for  
#pragma note noelle selected = 1 independent = 1 label = 4
      for (i = 0; i <= grid_points[0]-1; i++) {
...
#pragma omp for  
#pragma note noelle selected = 1 independent = 1 label = 5
      for (i = 1; i <= grid_points[0]-2; i++) {
        lhs[0][i][j][k] =   0.0;
  ...
  i = 1;
#pragma omp for nowait
#pragma note noelle selected = 1 independent = 1 label = 6
  for (j = 1; j <= grid_points[1]-2; j++) {
    for (k = 1; k <= grid_points[2]-2; k++) {
...
#pragma omp for nowait
#pragma note noelle selected = 1 independent = 1 label = 7
  for (i = 3; i <= grid_points[0]-4; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
  ...
  i = grid_points[0]-3;
#pragma omp for  
#pragma note noelle selected = 1 independent = 1 label = 8
  for (j = 1; j <= grid_points[1]-2; j++) {
    for (k = 1; k <= grid_points[2]-2; k++) {
      lhs[0][i][j][k] = lhs[0][i][j][k] + comz1;
      lhs[1][i][j][k] = lhs[1][i][j][k] - comz4;
  ...
  /*--------------------------------------------------------------------
    ...
    c-------------------------------------------------------------------*/
#pragma omp for  
#pragma note noelle selected = 1 independent = 1 label = 9
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
...
static void lhsy(void) {
  ...
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (k = 1; k <= grid_points[2]-2; k++) {
#pragma omp for  
#pragma note noelle selected = 1 independent = 1 label = 10
      for (j = 0; j <= grid_points[1]-1; j++) {
        ru1 = c3c4*rho_i[i][j][k];
        cv[j] = vs[i][j][k];
...
#pragma omp for  
#pragma note noelle selected = 1 independent = 1 label = 11
      for (j = 1; j <= grid_points[1]-2; j++) {
        lhs[0][i][j][k] =  0.0;
  ...
  j = 1;
#pragma omp for nowait
#pragma note noelle selected = 1 independent = 1 label = 12
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (k = 1; k <= grid_points[2]-2; k++) {
...
#pragma omp for nowait
#pragma note noelle selected = 1 independent = 1 label = 13
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 3; j <= grid_points[1]-4; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
        lhs[0][i][j][k] = lhs[0][i][j][k] + comz1;
  ...
  j = grid_points[1]-3;
#pragma omp for  
#pragma note noelle selected = 1 independent = 1 label = 14
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (k = 1; k <= grid_points[2]-2; k++) {
      lhs[0][i][j][k] = lhs[0][i][j][k] + comz1;
      lhs[1][i][j][k] = lhs[1][i][j][k] - comz4;
  ...
  /*--------------------------------------------------------------------
    ...
    c-------------------------------------------------------------------*/
#pragma omp for  
#pragma note noelle selected = 1 independent = 1 label = 15
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
        lhs[0+5][i][j][k]  = lhs[0][i][j][k];
...
static void lhsz(void) {
  ...
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
#pragma omp for  
#pragma note noelle selected = 1 independent = 1 label = 16
      for (k = 0; k <= grid_points[2]-1; k++) {
        ru1 = c3c4*rho_i[i][j][k];
        cv[k] = ws[i][j][k];
...
#pragma omp for  
#pragma note noelle selected = 1 independent = 1 label = 17
      for (k = 1; k <= grid_points[2]-2; k++) {
        lhs[0][i][j][k] =  0.0;
  ...
  k = 1;
#pragma omp for nowait
#pragma note noelle selected = 1 independent = 1 label = 18
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      lhs[2][i][j][k] = lhs[2][i][j][k] + comz5;
      lhs[3][i][j][k] = lhs[3][i][j][k] - comz4;
...
#pragma omp for nowait
#pragma note noelle selected = 1 independent = 1 label = 19
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 3; k <= grid_points[2]-4; k++) {
        lhs[0][i][j][k] = lhs[0][i][j][k] + comz1;
  ...
  k = grid_points[2]-3;
#pragma omp for  
#pragma note noelle selected = 1 independent = 1 label = 20
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      lhs[0][i][j][k] = lhs[0][i][j][k] + comz1;
      lhs[1][i][j][k] = lhs[1][i][j][k] - comz4;
  ...
  /*--------------------------------------------------------------------
    ...
    c-------------------------------------------------------------------*/
#pragma omp for  
#pragma note noelle selected = 1 independent = 1 label = 21
  for (i = 1; i <= grid_points[0]-2; i++) {
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
        lhs[0+5][i][j][k]  = lhs[0][i][j][k];
...
#pragma omp for nowait
#pragma note noelle selected = 1 independent = 1 label = 24
    for (i = 0; i <= grid_points[0]-1; i++) {
      for (j = 0; j <= grid_points[1]-1; j++) {
        for (k = 0; k <= grid_points[2]-1; k++) {
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 25
      for (i = 0; i <= grid_points[0]-1; i++) {
        for (j = 0; j <= grid_points[1]-1; j++) {
          for (k = 0; k <= grid_points[2]-1; k++) {
    ...
    /*--------------------------------------------------------------------
      ...
      c-------------------------------------------------------------------*/
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 26
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          uijk = us[i][j][k];
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 27
      for (j = 1; j <= grid_points[1]-2; j++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 28
      for (j = 1; j <= grid_points[1]-2; j++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          rhs[m][i][j][k] = rhs[m][i][j][k] - dssp * 
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 29
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 30
      for (j = 1; j <= grid_points[1]-2; j++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          rhs[m][i][j][k] = rhs[m][i][j][k] - dssp *
            ( u[m][i-2][j][k] - 4.0*u[m][i-1][j][k] + 
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 31
      for (j = 1; j <= grid_points[1]-2; j++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          rhs[m][i][j][k] = rhs[m][i][j][k] - dssp *
            ( u[m][i-2][j][k] - 4.0*u[m][i-1][j][k] +
...
#pragma omp barrier
    ...
    /*--------------------------------------------------------------------
      ...
      c-------------------------------------------------------------------*/
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 32
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          vijk = vs[i][j][k];
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 33
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          rhs[m][i][j][k] = rhs[m][i][j][k]- dssp * 
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 34
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          rhs[m][i][j][k] = rhs[m][i][j][k] - dssp * 
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 35
      for (i = 1; i <= grid_points[0]-2; i++) {
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 36
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          rhs[m][i][j][k] = rhs[m][i][j][k] - dssp *
            ( u[m][i][j-2][k] - 4.0*u[m][i][j-1][k] + 
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 37
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          rhs[m][i][j][k] = rhs[m][i][j][k] - dssp *
            ( u[m][i][j-2][k] - 4.0*u[m][i][j-1][k] +
...
#pragma omp barrier  
    ...
    /*--------------------------------------------------------------------
      ...
      c-------------------------------------------------------------------*/
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 38
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          wijk = ws[i][j][k];
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 39
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (j = 1; j <= grid_points[1]-2; j++) {
          rhs[m][i][j][k] = rhs[m][i][j][k]- dssp * 
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 40
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (j = 1; j <= grid_points[1]-2; j++) {
          rhs[m][i][j][k] = rhs[m][i][j][k] - dssp * 
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 41
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (j = 1; j <= grid_points[1]-2; j++) {
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 42
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (j = 1; j <= grid_points[1]-2; j++) {
          rhs[m][i][j][k] = rhs[m][i][j][k] - dssp *
            ( u[m][i][j][k-2] - 4.0*u[m][i][j][k-1] + 
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 43
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (j = 1; j <= grid_points[1]-2; j++) {
          rhs[m][i][j][k] = rhs[m][i][j][k] - dssp *
            ( u[m][i][j][k-2] - 4.0*u[m][i][j][k-1] +
    ...
    for (m = 0; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 44
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (j = 1; j <= grid_points[1]-2; j++) {
          for (k = 1; k <= grid_points[2]-2; k++) {
...
#pragma note noelle independent = 1
  {
    ...
    for (i = 0; i <= grid_points[0]-3; i++) {
      ...
      i2 = i  + 2;
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 47
      for (j = 1; j <= grid_points[1]-2; j++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
    ...
    i1 = grid_points[0]-1;
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 48
    for (j = 1; j <= grid_points[1]-2; j++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
    ...
    for (m = 3; m < 5; m++) {
      ...
      for (i = 0; i <= grid_points[0]-3; i++) {
        ...
        i2 = i  + 2;
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 49
        for (j = 1; j <= grid_points[1]-2; j++) {
          for (k = 1; k <= grid_points[2]-2; k++) {
            fac1               = 1./lhs[n+2][i][j][k];
            lhs[n+3][i][j][k]   = fac1*lhs[n+3][i][j][k];
...
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 50
      for (j = 1; j <= grid_points[1]-2; j++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          fac1               = 1./lhs[n+2][i][j][k];
          lhs[n+3][i][j][k]   = fac1*lhs[n+3][i][j][k];
    ...
    for (m = 0; m < 3; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 51
      for (j = 1; j <= grid_points[1]-2; j++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
    ...
    for (m = 3; m < 5; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 52
      for (j = 1; j <= grid_points[1]-2; j++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          n = (m-3+1)*5;
          rhs[m][i][j][k] = rhs[m][i][j][k] -
    ...
    for (i = grid_points[0]-3; i >= 0; i--) {
      ...
      i2 = i  + 2;
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 53
      for (m = 0; m < 3; m++) {
        for (j = 1; j <= grid_points[1]-2; j++) {
          for (k = 1; k <= grid_points[2]-2; k++) {
            rhs[m][i][j][k] = rhs[m][i][j][k] - 
    ...
    for (m = 3; m < 5; m++) {
      ...
      for (i = grid_points[0]-3; i >= 0; i--) {
        ...
        i2 = i  + 2;
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 54
        for (j = 1; j <= grid_points[1]-2; j++) {
          for (k = 1; k <= grid_points[2]-2; k++) {
            rhs[m][i][j][k] = rhs[m][i][j][k] - 
              lhs[n+3][i][j][k]*rhs[m][i1][j][k] -
...
#pragma note noelle independent = 1
  {
    ...
    for (j = 0; j <= grid_points[1]-3; j++) {
      ...
      j2 = j  + 2;
#pragma omp for      
#pragma note noelle selected = 1 independent = 1 label = 55
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          fac1               = 1./lhs[n+2][i][j][k];
          lhs[n+3][i][j][k]   = fac1*lhs[n+3][i][j][k];
    ...
    j1 = grid_points[1]-1;
#pragma omp for      
#pragma note noelle selected = 1 independent = 1 label = 56
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (k = 1; k <= grid_points[2]-2; k++) {
        fac1               = 1./lhs[n+2][i][j][k];
        lhs[n+3][i][j][k]   = fac1*lhs[n+3][i][j][k];
    ...
    for (m = 3; m < 5; m++) {
      ...
      for (j = 0; j <= grid_points[1]-3; j++) {
        ...
        j2 = j  + 2;
#pragma omp for      
#pragma note noelle selected = 1 independent = 1 label = 57
        for (i = 1; i <= grid_points[0]-2; i++) {
          for (k = 1; k <= grid_points[2]-2; k++) {
            fac1               = 1./lhs[n+2][i][j][k];
            lhs[n+3][i][j][k]   = fac1*lhs[n+3][i][j][k];
      ...
      j1 = grid_points[1]-1;
#pragma omp for      
#pragma note noelle selected = 1 independent = 1 label = 58
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          fac1               = 1./lhs[n+2][i][j][k];
          lhs[n+3][i][j][k]   = fac1*lhs[n+3][i][j][k];
    ...
    for (m = 0; m < 3; m++) {
#pragma omp for      
#pragma note noelle selected = 1 independent = 1 label = 59
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          rhs[m][i][j][k] = rhs[m][i][j][k] -
    ...
    for (m = 3; m < 5; m++) {
#pragma omp for      
#pragma note noelle selected = 1 independent = 1 label = 60
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (k = 1; k <= grid_points[2]-2; k++) {
          n = (m-3+1)*5;
          rhs[m][i][j][k] = rhs[m][i][j][k] -
    ...
    for (m = 0; m < 3; m++) {
      for (j = grid_points[1]-3; j >= 0; j--) {
        ...
        j2 = j  + 2;
#pragma omp for      
#pragma note noelle selected = 1 independent = 1 label = 61
        for (i = 1; i <= grid_points[0]-2; i++) {
          for (k = 1; k <= grid_points[2]-2; k++) {
            rhs[m][i][j][k] = rhs[m][i][j][k] - 
              lhs[n+3][i][j][k]*rhs[m][i][j1][k] -
    ...
    for (m = 3; m < 5; m++) {
      ...
      for (j = grid_points[1]-3; j >= 0; j--) {
        ...
        j2 = j1 + 1;
#pragma omp for      
#pragma note noelle selected = 1 independent = 1 label = 62
        for (i = 1; i <= grid_points[0]-2; i++) {
          for (k = 1; k <= grid_points[2]-2; k++) {
            rhs[m][i][j][k] = rhs[m][i][j][k] - 
              lhs[n+3][i][j][k]*rhs[m][i][j1][k] -
...
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 63
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
    ...
    k1 = grid_points[2]-1;
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 64
    for (i = 1; i <= grid_points[0]-2; i++) {
      for (j = 1; j <= grid_points[1]-2; j++) {
        fac1               = 1./lhs[n+2][i][j][k];
        lhs[n+3][i][j][k]   = fac1*lhs[n+3][i][j][k];
    ...
    for (m = 3; m < 5; m++) {
      n = (m-3+1)*5;
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 65
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (j = 1; j <= grid_points[1]-2; j++) {
          for (k = 0; k <= grid_points[2]-3; k++) {
            k1 = k  + 1;
      ...
      k1 = grid_points[2]-1;
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 66
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (j = 1; j <= grid_points[1]-2; j++) {
          fac1               = 1./lhs[n+2][i][j][k];
          lhs[n+3][i][j][k]   = fac1*lhs[n+3][i][j][k];
    ...
    for (m = 0; m < 3; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 67
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (j = 1; j <= grid_points[1]-2; j++) {
          rhs[m][i][j][k] = rhs[m][i][j][k] -
    ...
    for (m = 3; m < 5; m++) {
      n = (m-3+1)*5;
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 68
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (j = 1; j <= grid_points[1]-2; j++) {
          rhs[m][i][j][k] = rhs[m][i][j][k] -
            lhs[n+3][i][j][k]*rhs[m][i][j][k1];
    ...
    for (m = 0; m < 3; m++) {
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 69
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (j = 1; j <= grid_points[1]-2; j++) {
    ...
    for (m = 3; m < 5; m++) {
      n = (m-3+1)*5;
#pragma omp for
#pragma note noelle selected = 1 independent = 1 label = 70
      for (i = 1; i <= grid_points[0]-2; i++) {
        for (j = 1; j <= grid_points[1]-2; j++) {
          for (k = grid_points[2]-3; k >= 0; k--) {
            k1 = k  + 1;
