    ...
    char   *rand)
{
  ...
  printf( "    CLINKFLAGS   = %s\n", clinkflags );    printf( "    CLINKFLAGS   = %s\n", clinkflags );
...
#pragma note noelle independent = 1 selected = 1 isolated = 1 label = 1
  for (k = 0; k < d[2]; k++) {
    for (j = 0; j < d[1]; j++) {
      for (i = 0; i < d[0]; i++) {
...
#pragma note noelle independent = 1
  {
        dcomplex y0[NX][FFTBLOCKPAD];
        dcomplex y1[NX][FFTBLOCKPAD];
    int ffftblock = fftblock;

#pragma omp for 	
#pragma note noelle selected = 1 isolated = 1 label = 3
    for (k = 0; k < d[2]; k++) {
      for (jj = 0; jj <= d[1] - ffftblock; jj+=ffftblock) {
        dcomplex y0[NX][FFTBLOCKPAD];
        dcomplex y1[NX][FFTBLOCKPAD];
        /*          if (TIMERS_ENABLED == TRUE) timer_start(T_FFTCOPY); */
        for (j = 0; j < ffftblock; j++) {
          for (i = 0; i < d[0]; i++) {
...
#pragma omp for 	
#pragma note noelle selected = 1 isolated = 1 label = 4
    for (k = 0; k < d[2]; k++) {
      for (ii = 0; ii <= d[0] - ffftblock; ii+=ffftblock) {
        dcomplex y0[NX][FFTBLOCKPAD];
        dcomplex y1[NX][FFTBLOCKPAD];
        /*	    if (TIMERS_ENABLED == TRUE) timer_start(T_FFTCOPY); */
        for (j = 0; j < d[1]; j++) {
          for (i = 0; i < ffftblock; i++) {
...
#pragma note noelle independent = 1 
  {
    ...
    int ffftblock = fftblock;
#pragma omp for 	
#pragma note noelle selected = 1 isolated = 1 label = 5
    for (j = 0; j < d[1]; j++) {
      for (ii = 0; ii <= d[0] - ffftblock; ii+=ffftblock) {
        dcomplex y0[NX][FFTBLOCKPAD];
        dcomplex y1[NX][FFTBLOCKPAD];
