...
c-------------------------------------------------------------------*/
#pragma omp for nowait
#pragma note noelle independent = 1 selected = 1 label = 2
    for (i = 1; i <= NA+1; i++) {
	x[i] = 1.0;
    }
#pragma omp for nowait
#pragma note noelle independent = 1 selected = 1 label = 3
...
#pragma note noelle independent = 1 selected = 1 label = 4
  for (j = 1; j <= lastcol-firstcol+1; j++) {
...
#pragma note noelle independent = 1 selected = 1 label = 5
  for (j = 1; j <= lastcol-firstcol+1; j++) {
            x[j] = norm_temp12*z[j];
	}
...
#pragma note noelle independent = 1 selected = 1 label = 6
    for (i = 1; i <= NA+1; i++) {
         x[i] = 1.0;
    }  
    zeta  = 0.0;
...
#pragma omp parallel for default(shared) private(j) reduction(+:norm_temp11,norm_temp12)
#pragma note noelle independent = 1 selected = 1 label = 7
	for (j = 1; j <= lastcol-firstcol+1; j++) {
            norm_temp11 = norm_temp11 + x[j]*z[j];
            norm_temp12 = norm_temp12 + z[j]*z[j];
	}
...
c-------------------------------------------------------------------*/
#pragma omp parallel for default(shared) private(j)
#pragma note noelle independent = 1 selected = 1 label = 8
	for (j = 1; j <= lastcol-firstcol+1; j++) {
            x[j] = norm_temp12*z[j];
	}
...
#pragma note noelle independent = 1 label = 11
{
...
/* rolled version */    
#pragma omp for 
#pragma note noelle selected = 1 isolated = 1 label = 12
  for (j = 1; j <= lastrow-firstrow+1; j++) {
            sum = 0.0;
	    for (k = rowstr[j]; k < rowstr[j+1]; k++) {
...
#pragma note noelle selected = 1 solated = 1 label = 16 
  for (j = 1; j <= lastrow-firstrow+1; j++) {
	d = 0.0;
...
c---------------------------------------------------------------------*/
#pragma omp parallel for default(shared) private(i)
#pragma note noelle independent = 1 selected = 1 label = 18
    for (i = 1; i <= n; i++) {
	colidx[n+i] = 0;
    }
...
c-------------------------------------------------------------------*/
#pragma omp parallel for default(shared) private(j)
#pragma note noelle independent = 1 selected = 1 label = 19
    for (j = 1; j <= n; j++) {
	rowstr[j] = 0;
	mark[j] = FALSE;
    }
...
#pragma note noelle independent = 1 selected = 1 label = 20
      for(j = 0;j <= nrows-1;j++) {
         for(k = rowstr[j];k <= rowstr[j+1]-1;k++)
	       a[k] = 0.0;
      }
...
c-------------------------------------------------------------------*/
    nza = 0;
#pragma omp parallel for default(shared) private(i)    
//#pragma note noelle independent = 1 selected = 1 label = 21
    for (i = 1; i <= n; i++) {
	x[i] = 0.0;
	mark[i] = FALSE;
    }
