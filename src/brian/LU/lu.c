...
#pragma note noelle selected = 1 label = 1 isolated = 1
  for (i = ist; i <= iend; i++) {
    for (j = jst; j <= jend; j++) {
      for (m = 0; m < 5; m++) {
	v[i][j][k][m] = v[i][j][k][m]
...
#pragma omp for nowait schedule(static)
#pragma note noelle selected = 1 label = 2 isolated = 1
  for (i = ist; i <= iend; i++) {
...
#pragma omp for nowait schedule(static)
#pragma note noelle  selected = 1 label = 3 isolated = 1
  for (i = iend; i >= ist; i--) {
    for (j = jend; j >= jst; j--) {
      for (m = 0; m < 5; m++) {
	tv[i][j][m] = 
...
#pragma omp for nowait schedule(static)
#pragma note noelle selected = 1 label = 4 isolated = 1
  for (i = iend; i >= ist; i--) {
#if defined(_OPENMP)      
    if (i != iend) {
      while (flag[i+1] == 0) {
...
#pragma omp for
#pragma note noelle selected = 1 label = 5
  for (i = 0; i < nx; i++) {
    for (j = 0; j < ny; j++) {
      for (k = 0; k < nz; k++) {
	for (m = 0; m < 5; m++) {
...
#pragma omp for nowait schedule(static)
#pragma note noelle  selected = 1 label = 12 isolated = 1 independent = 1
  for (i = ist; i <= iend; i++) {
    for (j = jst; j <= jend; j++) {
...
#pragma omp for nowait schedule(static)
#pragma note noelle  selected = 1 label = 14 isolated = 1 independent = 1
#if defined(_OPENMP)  
  for (i = iend; i >= ist; i--) {
      for (j = jend; j >= jst; j--) {
#else	  
...
#pragma omp for  
  #pragma note noelle selected = 1 label = 16 isolated = 1
  for (i = 0; i <= nx-1; i++) {
    for (j = 0; j <= ny-1; j++) {
      for (k = 0; k <= nz-1; k++) {
	for (m = 0; m < 5; m++) {
...
#pragma omp for
  #pragma note noelle selected = 1 label = 17 isolated = 1
  for (i = L1; i <= L2; i++) {
    for (j = jst; j <= jend; j++) {
      for (k = 1; k <= nz - 2; k++) {
...
#pragma omp for  
  #pragma note noelle selected = 1 label = 18 isolated = 1
  for (j = jst; j <= jend; j++) {
    for (k = 1; k <= nz - 2; k++) {
      for (i = ist; i <= iend; i++) {
	for (m = 0; m < 5; m++) {
...
#pragma omp for  
  #pragma note noelle selected = 1 label = 19 isolated = 1
  for (i = ist; i <= iend; i++) {
    for (j = L1; j <= L2; j++) {
      for (k = 1; k <= nz - 2; k++) {
...
#pragma omp for  
  #pragma note noelle selected = 1 label = 20 isolated = 1
  for (i = ist; i <= iend; i++) {
    for (k = 1; k <= nz - 2; k++) {
      for (j = jst; j <= jend; j++) {
	for (m = 0; m < 5; m++) {
...
--------------------------------------------------------------------*/
#pragma omp for  
  #pragma note noelle selected = 1 label = 21 isolated = 1
  for (i = ist; i <= iend; i++) {
    for (j = jst; j <= jend; j++) {
      for (k = 0; k <= nz-1; k++) {
...
--------------------------------------------------------------------*/
#pragma omp for
  #pragma note noelle selected = 1 label = 23
  for (i = 0; i < nx; i++) {
    iglob = i;
    for (k = 0; k < nz; k++) {
...
--------------------------------------------------------------------*/
#pragma omp for
  #pragma note noelle selected = 1 label = 25
  for (j = 0; j < ny; j++) {
    jglob = j;
    for (k = 0; k < nz; k++) {
...
#pragma omp for
  #pragma note noelle selected = 1 label = 26
  for (j = 0; j < ny; j++) {
    jglob = j;
    for (k = 0; k < nz; k++) {
...
--------------------------------------------------------------------*/
#pragma omp for    
  #pragma note noelle selected = 1 label = 29 isolated = 1
    for (i = ist; i <= iend; i++) {
      for (j = jst; j <= jend; j++) {
	for (k = 1; k <= nz - 2; k++) {
	  for (m = 0; m < 5; m++) {
...
#pragma omp for
  #pragma note noelle selected = 1 label = 30 isolated = 1
    for (i = ist; i <= iend; i++) {
      for (j = jst; j <= jend; j++) {
	for (k = 1; k <= nz-2; k++) {
	  for (m = 0; m < 5; m++) {
